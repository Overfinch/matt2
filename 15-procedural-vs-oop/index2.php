...
// ParamHandler::getInstance() единоразово проверяет формат, и мы не обязаны потом делать это в каждой функции, как это было в процидурном примере
// реализация методов упущена для того что бы код был нагляднее

// если нам понадобится внести новый формат, то мы сможем просто добавить новый подкласс, и реализовать это в нём
...
$test->read(); // Вызывая метод read мы в нём не проверяем формат, это уже сделал сам класс ParamHandler, ответственность мы перекладывваем на него
